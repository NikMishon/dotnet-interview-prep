...
## 8. Асинхронный синглтон
Паттерн для создания синглтона, инициализация которого происходит асинхронно.

- **Проблема**: Стандартная реализация `Singleton` с использованием `Lazy<T>` не подходит для асинхронной инициализации. Если несколько потоков одновременно обратятся к `Lazy<T>.Value`, один поток заблокирует другие на время выполнения фабричного метода. В асинхронном коде это может привести к дедлокам или неэффективному использованию потоков.
- **Решение**: Использовать `AsyncLazy<T>`, который оборачивает `Task<T>` и позволяет асинхронно ожидать результат инициализации без блокировки потоков.

```csharp
// Вспомогательный класс для асинхронной ленивой инициализации
public class AsyncLazy<T> : Lazy<Task<T>>
{
    public AsyncLazy(Func<Task<T>> taskFactory) : base(taskFactory) { }
}

public class MyAsyncSingleton
{
    private static readonly AsyncLazy<MyAsyncSingleton> _instance = 
        new AsyncLazy<MyAsyncSingleton>(async () => 
        {
            var instance = new MyAsyncSingleton();
            await instance.InitializeAsync();
            return instance;
        });

    private MyAsyncSingleton() { }

    private async Task InitializeAsync()
    {
        // Здесь происходит асинхронная инициализация
        await Task.Delay(1000); // Например, загрузка данных из сети
    }

    public static Task<MyAsyncSingleton> GetInstanceAsync()
    {
...
## 9. Паттерн IDisposable, исключающий повторный вызов
Классический паттерн для корректного освобождения ресурсов и защиты от двойного вызова `Dispose()`.

- **Цель**: Гарантировать, что управляемые и неуправляемые ресурсы будут освобождены ровно один раз.
- **Ключевые элементы**:
    - Приватный флаг `_disposed`.
    - Публичный метод `Dispose()`.
    - Защищенный виртуальный метод `Dispose(bool disposing)`.
    - Финализатор (деструктор `~`), если класс владеет неуправляемыми ресурсами.

```csharp
public class ResourceHolder : IDisposable
{
    private bool _disposed = false; // Флаг для отслеживания вызовов

    // Публичный метод, который могут вызывать пользователи класса
    public void Dispose()
    {
        Dispose(true);
        // Запрещаем сборщику мусора вызывать финализатор для данного объекта,
        // так как ресурсы уже освобождены.
        GC.SuppressFinalize(this);
    }

    // Защищенный метод, содержащий основную логику очистки
    protected virtual void Dispose(bool disposing)
    {
        // Проверяем, не были ли ресурсы уже освобождены
        if (_disposed)
        {
            return;
        }
        ...
        if (disposing)
        {
            // Освобождаем управляемые ресурсы (другие объекты .NET, имеющие Dispose)
            // component.Dispose();
        }

        // Освобождаем неуправляемые ресурсы (файловые дескрипторы, COM-объекты)
        // CloseHandle(handle);

        _disposed = true;
    }

    // Финализатор вызывается сборщиком мусора, если пользователь "забыл" вызвать Dispose().
    // Его следует реализовывать ТОЛЬКО если класс напрямую владеет неуправляемыми ресурсами.
    ~ResourceHolder()
    {
        Dispose(false);
    }
